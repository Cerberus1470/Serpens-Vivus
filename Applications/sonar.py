...
def enterPlayerMove(previousMoves):
    ...
    while True:
        ...
        move = move.split()
        if len(move) == 2 or move[0].isdigit() or move[1].isdigit() or isOnBoard(int(move[0]), int(move[1])):
...
while True:
    ...
    theChests = getRandomChests(chests)
    drawBoard(theBoard)
    previousMoves = []

    while devices > 0:
        # Show sonar device and chest statuses.
        print('You have %s sonar device(s) left. %s treasure chest(s) remaining.' % (devices, len(theChests)))
